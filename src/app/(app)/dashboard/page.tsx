...
import { CalendarDays, History, ListChecks, ShieldAlert, FlaskConical, Edit3, Clock, Trash2, AlertTriangle, BrainCircuit, Loader2, Star } from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const { deleteAccount: authDeleteAccount, currentUser, userProfile, isLoading: isAuthLoading } = useAuth();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const records = recordsSnapshot.docs.map(doc => ({ ...doc.data(), id: doc.id })) as HealthRecord[];
  ...
  }, [currentUser, toast, isAuthLoading]);
  ...
  const handleDeleteAccount = async () => {
    ...
    if (result.success) {
      toast({
        ...
        description: "Your account has been removed. We are sorry to see you go.",
      });
    } else {
      toast({
        variant: "destructive",
  ...
  if (!isClient || isFetchingData || isAuthLoading) {
    return (
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <div className="relative">
                            <Button onClick={handleGenerateSummary} disabled={healthRecords.length === 0 || isFreePlan}>
                        ...
                        {isFreePlan && (
                          <TooltipContent>
                            <p>Upgrade to Standard to unlock AI Summaries.</p>
                          </TooltipContent>
                        )}
                      </Tooltip>
                    </TooltipProvider>
                     {(healthRecords.length === 0 && !isFreePlan) && <p className="text-xs text-muted-foreground mt-2">You have no records to summarize yet.</p>}
                     {isFreePlan && 
                        <div className="text-xs text-muted-foreground mt-2 flex items-center gap-2">
                           <p>Feature disabled on Free plan.</p>
                           <Button variant="link" size="sm" asChild className="p-0 h-auto text-xs"><Link href="/pricing">Upgrade <Star className="ml-1 h-3 w-3 fill-yellow-400 text-yellow-500" /></Link></Button>
                        </div>
                     }
                </div>
            )}
                ...
                <div className="space-y-6">
                  {healthRecords.map((record) => (
                    <Card key={record.id} className="bg-secondary/30 shadow-md hover:shadow-lg transition-shadow">
                      <CardHeader>
                        <CardTitle className="text-lg">Analysis on {format(new Date(record.date), "MMMM d, yyyy")}</CardTitle>
                      </CardHeader>
      ...
      <Card className="shadow-lg mt-8">
           ...
           <AlertDialog>
            ...
            <AlertDialogContent>
              <AlertDialogHeader>
                ...
                <AlertDialogDescription>
                  This action cannot be undone. This will permanently delete your account. Your health data stored in our database will be disassociated and inaccessible.
                  In a production system, this data would be permanently deleted or anonymized according to privacy regulations.
                </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter>
        ...
        <CardFooter>
          <p className="text-xs text-muted-foreground">
            Account deletion in this prototype only removes your user account.
            A production system would have robust, compliant data deletion processes for all associated data.
          </p>
        </CardFooter>
      </Card>
    </div>
